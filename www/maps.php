						...
						if (gid != usergid) {
							if (pending == 2){
								...
								$(".accept-request-button-"+i).button({
									text: true
								}).click(function(){acceptRequest(userId,uid);document.location.reload(True);});
							}
							else{
				...
				$person = $xml.find( "response person" ).each(
					function(){
						...
						if (gid != usergid) {
							...
							$(".icon-button-"+i).button({ icons: { primary: "ui-icon-circle-plus" }, text: false }).click(function(){sendRequest(userId,uid);document.location.reload(True);});
						}
						i++;
					}
				);		
